...
| Styling | Tailwind CSS | 4 | Soft water-tinted background with white cards (all tokens on hue 225). Light and dark themes chosen by the user: `data-theme` on `<html>` drives the tokens and the `dark:` variant, an inline script in the root layout applies the saved choice before the first paint, and "system" follows the OS. Preference in `localStorage`, toggle in the header (`src/lib/theme.ts`, `src/hooks/use-theme.ts`) |
