...
| Languages | Own dictionary (`src/lib/i18n.ts`) | — | English, Greek and Italian, matching the countries of the OAH pilot sites. `lang` on `<html>` is the source of truth, set before the first paint by an inline script and followed by `useSyncExternalStore` (`src/hooks/use-locale.ts`); first visit follows `navigator.languages`, the choice is kept in `localStorage`, and the toggle sits in the header. Scope is the citizen surface — navigation, the report flow, the map sidebar. Clinician pages and the alert text generated by the API stay English, since that text is written by the risk engine, not the UI. Geist has no Greek subset, so `:root[lang="el"]` switches to Noto Sans, which is not preloaded |
