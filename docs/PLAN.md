...
- **Next (user):** put a free Gemini API key on the host (`GEMINI_API_KEY` in `/srv/env/stream-to-clinic.env`, then restart the API) so the clinic-desk notice works in the video — until then the endpoint answers 503 and the panel says so; record the demo video from [DEMO_SCRIPT.md](DEMO_SCRIPT.md), add its link to [SUBMISSION.md](SUBMISSION.md), submit on Devpost before Sep 30, 9:00 PM PDT.
